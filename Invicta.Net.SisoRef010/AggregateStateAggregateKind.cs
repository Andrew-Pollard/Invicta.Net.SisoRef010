//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

// 
//Copyright © 2010-2011, 2013-2021 by the Simulation Interoperability Standards Organization, Inc.
//7901 4th St N, Suite 300-4043
//St. Petersburg, FL 33702
//All rights reserved.
//
//Schema and API: SISO hereby grants a general, royalty-free license to copy, distribute, display, and
//make derivative works from this material, for all purposes, provided that any use of the material
//contains the following attribution: “Reprinted with permission from SISO Inc.” Should
//a reader require additional information, contact the SISO Inc. Board of Directors.
//
//Documentation: SISO hereby grants a general, royalty-free license to copy, distribute,
//display, and make derivative works from this material, for noncommercial purposes, provided that
//any use of the material contains the following attribution: “Reprinted with permission from SISO Inc.” 
//The material may not be used for a commercial purpose without express written
//permission from the SISO Inc Board of Directors.
//
//SISO Inc. Board of Directors
//7901 4th St N, Suite 300-4043
//St. Petersburg, FL 33702
//
namespace Invicta.Net.SisoRef010 {
	
	[Invicta.Net.SisoRef010.Attributes.SisoEnumerationAttribute(206, "Aggregate State-Aggregate Kind", 8)]
	public enum AggregateStateAggregateKind : byte {
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1a6fae-526a-11df-b84e-080069138b88", Description="Other")]
		Other = 0,
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1b7e30-526a-11df-aad9-080069138b88", Description="Military Hierarchy")]
		MilitaryHierarchy = 1,
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1c9036-526a-11df-a839-080069138b88", Description="Common Type")]
		CommonType = 2,
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1da368-526a-11df-83be-080069138b88", Description="Common Mission")]
		CommonMission = 3,
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1ead58-526a-11df-a9e5-080069138b88", Description="Similar Capabilities")]
		SimilarCapabilities = 4,
		[Invicta.Net.SisoRef010.Attributes.SisoEnumerantAttribute("1b1fbf54-526a-11df-9177-080069138b88", Description="Common Location")]
		CommonLocation = 5,
	}
}
